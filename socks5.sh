...
CLI_HOST=""
CLI_DUAL_STACK=0
REMOTE_SCRIPT_URL="https://raw.githubusercontent.com/yayitinyu/socks/main/socks5.sh"
...
rollback_install() {
    ...
    rm -f -- "$SERVICE_UNIT" "$FIREWALL_UNIT" "$FIREWALL_HELPER" "$DANTE_CONFIG_FILE" "/usr/local/bin/${APP_NAME}"
    if command -v systemctl >/dev/null 2>&1; then
...
print_runtime_status() {
    ...
    local display_host ipv4_host ipv6_host dante_version_output dante_version selinux_status="未启用"
...
install_action() {
    ...
    if [[ -n "$CLI_PORT" ]]; then
        ...
        if ((CLI_FORCE == 1)) && port_in_use "$SOCKS_PORT"; then
            pkill -9 -f "socks5-node" 2>/dev/null || true
